...
import lmdb
import torch
env = lmdb.open("/home/jn/code-pytorch/im2re-pytor-jn-NEW/im2recipe-Pytorch-master/data/train_lmdb", max_readers=1, readonly=True, lock=False, readahead=True, meminit=False)
txn = env.begin()
with open('/home/jn/code-pytorch/im2re-pytor-jn-NEW/im2recipe-Pytorch-master/data/train_keys.pkl', 'rb') as f:
    ids = pickle.load(f)
...
for i in range(len(ids)):
     ...
     if class_count[sample['classes']] > 2900:
         if keys_class_count[sample['classes']] <= 2000:
             keys.append(ids[i])
         ...
         if keys_class_count[sample['classes']] <= class_count[sample['classes']]*0.3:
             keys.append(ids[i])
             keys_class_count[sample['classes']] = keys_class_count[sample['classes']] + 1
     elif class_count[sample['classes']] < 100 and class_count[sample['classes']] >= 50:
         if keys_class_count[sample['classes']] <= class_count[sample['classes']]*0.6:
             keys.append(ids[i])
             keys_class_count[sample['classes']] = keys_class_count[sample['classes']] + 1
     elif class_count[sample['classes']] < 50:
         if keys_class_count[sample['classes']] <= 50:
             keys.append(ids[i])
             keys_class_count[sample['classes']] = keys_class_count[sample['classes']] + 1
...
print(keys)
print(len(keys))
pickle.dump(keys,open("num_pkl.pkl",'wb'))
#
