...
import lmdb
import torch
env = lmdb.open("/home/jn/code-pytorch/im2re-pytor-jn-NEW/im2recipe-Pytorch-master/data/val_lmdb", max_readers=1, readonly=True, lock=False, readahead=True, meminit=False)
txn = env.begin()
with open('/home/jn/code-pytorch/im2re-pytor-jn-NEW/im2recipe-Pytorch-master/data/val_keys.pkl', 'rb') as f:
    ids = pickle.load(f)
...
for x in keys:
    if not x in d:
        ...
        d[x] = d[x] + 1
print(d)
pickle.dump(keys,open("val_small_pkl.pkl",'wb'))
